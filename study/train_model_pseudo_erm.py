...
from src.data.pseudolabel import GroundTruthPseudolabelsDataset, split_and_pseudolabel_dataset
from study.utils.loaders import load_model, load_dataset, load_loss, load_optimizer, load_lr_schedule, load_pretrained_model, load_metrics
...
def train_model_pseudo_erm(cfg: DictConfig):
    print(f"Training model using {100 * cfg.model_training.labeled_ratio:.1f}% of the labeled dataset and pseudo-ERM learning...")
    ...
    gt_pl_pseudolabeled_dataset_tr = GroundTruthPseudolabelsDataset(pseudolabeled_dataset=pl_pseudolabeled_dataset_tr)
    gt_pseudolabeled_dataset_tr = torch.utils.data.ConcatDataset([l_pseudolabeled_dataset_tr, gt_pl_pseudolabeled_dataset_tr])
    gt_pseudolabeled_dataloader_tr = DataLoader(
        gt_pseudolabeled_dataset_tr,
        batch_size=get_batch_size(gt_pseudolabeled_dataset_tr, cfg.pretrained_model.batch_size),
        shuffle=True,
        num_workers=cfg.study.num_workers,
    )
    ...
    if ("reduction" in cfg.loss.kwargs) and (cfg.loss.kwargs.reduction != "sum"):
        print("WARNING: GT-pseudolabels training is only valid for loss reduction set to 'sum'...")
    ...
    n_batches_per_epoch = len(gt_pseudolabeled_dataloader_tr)
    ...
    for epoch in range(n_epochs):
        ...
        for batch_idx, (inputs, labels, _, _) in enumerate(gt_pseudolabeled_dataloader_tr):
            ...
            for (inputs_chunk, labels_chunk) in iterate_on_chunks(
                ...
                # Forward pass
                preds_chunk = model(inputs_chunk)
                loss = criterion(preds_chunk, labels_chunk) / n_datapoints_batch
